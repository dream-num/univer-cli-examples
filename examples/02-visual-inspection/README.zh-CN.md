...
这个 example 完整保留 01 基于 SQLite 的 Sheet、Doc、Slide Server、Web 文件侧边栏和 CLI 主链路，只增加视觉
检查：从 Server 拉取最新 UnitData，交给 browser-backed Render Runtime，并写出 PNG。
...
- `src/cli/program.ts`：在 01 的 commands 后增加 `screenshot` 和 `lint`。
