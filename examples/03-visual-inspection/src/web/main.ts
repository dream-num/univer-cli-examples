...
const unitId = new URL(location.href).searchParams.get("unit");
...
if (unitId !== null) {
  ...
  await mountEditor(unitId, parseUnitType(unit.unitType));
}
...
async function mountEditor(unitId: string, unitType: UnitType): Promise<void> {
  document.querySelector<HTMLElement>("#empty")!.style.display = "none";
  document.querySelector<HTMLElement>("#app")!.style.display = "block";
  ...
  document.querySelector("#status")!.textContent = `${unitType} · ${unitId} · connected`;
}
