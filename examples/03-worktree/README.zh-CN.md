...
这个 example 完整保留 02 的 Server、Web、CLI 和视觉检查，只增加 Worktree：Agent 在隔离 draft 中修改，标记
...
- `src/cli/features/unit.ts` 与 `visual.ts`：open、screenshot 和 lint 可查看 trunk 或 Worktree。
...
其余内容继承 02。本例不增加 runtime pool、worker 或 daemon。
